class Start extends Phaser.Scene {
    ...
    preload() {
        ...
        this.load.spritesheet("spaceShip", "Player/playerShip.png", { frameWidth: 16, frameHeight: 23 });
    ...
    create() {
        ...
        this.anims.create({
            key: 'shipAnim',
            frames: this.anims.generateFrameNumbers('spaceShip', { start: 3, end: 5 }),
            frameRate: 20,
            repeat: -1
        });
        ...
        this.spaceShip = this.add.sprite(config.width/2, 200, "spaceShip").setScale(2.5).play('shipAnim');

        this.add.text(config.width/2 - 80 , 240, 'Space', { fontFamily: '"Press Start 2P"', fontSize: 32, color: 'red' }).setStroke('#fff', 16);
        this.add.text(config.width/2 - 56 , 280, 'Hero', { fontFamily: '"Press Start 2P"', fontSize: 32, color: 'beige' }).setStroke('red', 16);
        let playLabel = this.add.text(config.width/2 - 114 , 380, 'Press ENTER to PLAY', { fontFamily: '"Press Start 2P"', fontSize: 12, color: 'green' });
